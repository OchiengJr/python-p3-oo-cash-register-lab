...
class TestCashRegister:
    ...
    def test_discount_attribute(self):
        ...
        assert self.cash_register.discount == 0, "Default discount should be 0"
        assert self.cash_register_with_discount.discount == 20, "Discount should be set to 20"

    def test_total_attribute(self):
        '''sets an instance variable total to zero on initialization.'''
        assert self.cash_register.total == 0, "Initial total should be 0"
        assert self.cash_register_with_discount.total == 0, "Initial total should be 0"

    def test_items_attribute(self):
        '''sets an instance variable items to empty list on initialization.'''
        assert self.cash_register.items == [], "Initial items should be an empty list"
        assert self.cash_register_with_discount.items == [], "Initial items should be an empty list"

    def test_add_item(self):
        '''accepts a title and a price and increases the total.'''
        self.cash_register.add_item("eggs", 0.98)
        assert self.cash_register.total == 0.98, "Total should be updated to 0.98"
        self.reset_register_totals()

    def test_add_item_optional_quantity(self):
        '''also accepts an optional quantity.'''
        self.cash_register.add_item("book", 5.00, 3)
        assert self.cash_register.total == 15.00, "Total should be updated to 15.00"
        self.reset_register_totals()

    def test_add_item_with_multiple_items(self):
        '''doesn’t forget about the previous total.'''
        self.cash_register.add_item("Lucky Charms", 4.5)
        assert self.cash_register.total == 4.5, "Total should be updated to 4.5 after first item"
        self.cash_register.add_item("Ritz Crackers", 5.0)
        assert self.cash_register.total == 9.5, "Total should be updated to 9.5 after second item"
        self.cash_register.add_item("Justin's Peanut Butter Cups", 2.50, 2)
        assert self.cash_register.total == 14.5, "Total should be updated to 14.5 after third item"
        self.reset_register_totals()
    ...
    def test_apply_discount(self):
        ...
        assert self.cash_register_with_discount.total == 800, "Total should be 800 after applying discount"
        self.reset_register_totals()
    ...
    def test_items_list_without_multiples(self):
        ...
        assert new_register.items == ["eggs", "tomato"], "Items list should contain 'eggs' and 'tomato'"
    ...
    def test_items_list_with_multiples(self):
        ...
        assert new_register.items == ["eggs", "eggs", "tomato", "tomato", "tomato"], "Items list should contain 'eggs' twice and 'tomato' thrice"
    ...
    def test_void_last_transaction(self):
        ...
        assert self.cash_register.total == 0.99, "Total should be 0.99 after voiding last transaction"
        self.reset_register_totals()
    ...
    def test_void_last_transaction_with_multiples(self):
        ...
        self.cash_register.void_last_transaction()
        assert self.cash_register.total == 0.0, "Total should be 0.0 after voiding last transaction with multiple items"
        self.reset_register_totals()
